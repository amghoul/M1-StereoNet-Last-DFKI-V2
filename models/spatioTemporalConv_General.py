...
## assume stream =1 and supspace = 1 (conv2d+conv1d)    
'''
class SpatioTemporalConv(nn.Module):
    def __init__(self, stream,ch_in,ch_out,model_bn,islastBlock=False, BN_1D=1, BN_2D=0, bias=False): #BN_1D BN_2D values: 1 or 0 i.e: use BN with conv2D or Conv1D
        super().__init__()
            ...
            if model_bn == 1: ## enable BN according to varaibles BN_1D & BN_2D values
                for subs in range(len(stream.kernels)):
                    if subs == 0 or subs ==len(stream.kernels)-1 : # first or last conv
                        if subs == 0: # first conv2d
                            self.spat_temp_conv.add_module('conv2d_subs'+str(subs), nn.Conv3d(ch_in, stream_out_chs[subs], stream_kernel_sizes[subs], 
                            ...
                            self.spat_temp_conv.add_module('conv1d_subs'+str(subs), nn.Conv3d(stream_out_chs[subs-1], ch_out, stream_kernel_sizes[subs], 
                                                            stream_strides[subs], stream_paddings[subs]))
                            if BN_1D ==1:
                        ...
                        self.spat_temp_conv.add_module('conv2d_subs'+str(subs), nn.Conv3d(stream_out_chs[subs-1], stream_out_chs[subs], stream_kernel_sizes[subs], 
                                                        stream_strides[subs], stream_paddings[subs]))
